        ...
        <div class="row text-center">
            <div class="column">
                <div class="report-record-highlight" v-if="resultsOfficials.length > 0">
                    <svg class="icon icon-person"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-person"></use></svg>
  ...
  methods: {
    ...
    getResults: function() {
      ...
      if (
        ...
        _this.$parent.sharedState.show.indexOf("voting") !== -1
      ) {
        // voting precincts
        _this.apiFetch(
          {
            ...
            limit: 1,
            columns: `voting_precincts.cc, voting_precincts.school, polling_locations.name as label,polling_locations.address,voting_precincts.precno as precinct,st_x(st_transform(polling_locations.the_geom, 4326)) as lng, st_y(st_transform(polling_locations.the_geom, 4326)) as lat, ST_Distance(polling_locations.the_geom,ST_Transform(GeomFromText('POINT(${Number(
              _this.$parent.sharedState.selected.lnglat[0]
            )} ${Number(
              _this.$parent.sharedState.selected.lnglat[1]
            )})',4326), 2264)) as distance`,
            join:
              "polling_locations;voting_precincts.precno = polling_locations.precno"
          },
          `https://mcmap.org/api/intersect_point/v1/voting_precincts/${_this.$parent.sharedState.selected.lnglat.join(
            ","
          )}/4326`,
          "resultsPrecinct"
        );
        ...
        _this.apiFetch(
          ...
          `https://mcmap.org/api/intersect_point/v1/national_congressional/${_this.$parent.sharedState.selected.lnglat.join(
            ","
          )}/4326`,
          "resultsNatlcong"
        );
        ...
        _this.apiFetch(
          ...
          `https://mcmap.org/api/intersect_point/v1/state_senate/${_this.$parent.sharedState.selected.lnglat.join(
            ","
          )}/4326`,
          "resultsNCSenate"
        );
        ...
        _this.apiFetch(
          ...
          `https://mcmap.org/api/intersect_point/v1/state_house/${_this.$parent.sharedState.selected.lnglat.join(
            ","
          )}/4326`,
          "resultsNCHouse"
        );
        ...
        _this.apiFetch(
          ...
          `https://mcmap.org/api/intersect_point/v1/city_council/${_this.$parent.sharedState.selected.lnglat.join(
            ","
          )}/4326`,
          "resultsCharlotte"
        );
